...
<body>
  ...
  <main>
    ...
    <div class="checkbox-center">
      <div>
        <form action="ogp_act2.php" method="post">
          <ul>
          ...
          <label for="GET-name">
            ...
            <div class="center">
              <button class="simple_square_btn1">
                <a href="ogp_act2.php">
                  <input type="submit" value="" />送信する</input></a>
                </button>
              </div>
              <br>
              <br>
  ...
  // 興味・関心のある分野
  var input_interestings = document.querySelectorAll("input[name=job_category]");

  if( 0 < input_interestings.length ) {

	checked_data['job_category'] = new Array();

	for(var data of input_interestings) {
		if( data.checked ) {
			checked_data['job_category'].push(data.value);
		}
	}
}
